    ...
    const handleRequest = async (rideId, userId, action) => {
        try {
          ...
          setPendingRequests(prev =>
            prev.filter(r => !(r._id === rideId && r.requests[0]._id === userId))
          );
                  ...
                  <div>
                    ...
                    <p>On: {new Date(p.rideDate).toLocaleDateString()}</p>
                      ...
                      <button
                        ...
                        onClick={() => handleRequest(p._id, p.requests[0]._id, 'approve')}
                      >
                        Approve
                      </button>
                      <button
                        className="bg-red-600 px-3 py-1 rounded"
                        onClick={() => handleRequest(p._id, p.requests[0]._id, 'deny')}
                      >
                        Deny
                      </button>
                    </div>
